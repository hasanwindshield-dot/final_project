...
export const MovieTagInput = ({
  movieTags,
  setMovieTags,
}: {
  movieTags: string[];
  setMovieTags: Dispatch<SetStateAction<string[]>>;
}) => {
  ...
  const handleAddTagFromNoOptions = () => {
    ...
    if (trimmedValue && !movieTags.includes(trimmedValue)) {
      setMovieTags((prev) => [...prev, trimmedValue]);
      setInputValue('');
    }
  };
  ...
  return (
    <AsyncSelect
      ...
      value={movieTags?.map((tag) => ({ label: tag, value: tag }))}
