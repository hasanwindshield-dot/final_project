...
export const RecievedCommentsPage = () => {
  const tableColumns = [
    'Name',
    'Prop',
    'Comment',
  ...
  const getCommentsList = async (showLoader = true, fetchMore = false) => {
    ...
    try {
      const { data } = await request.post('/comments/received', {
        limit: 10,
                        ...
                        >
                          <img
                            ...
                            className={`w-[34px] h-[34px] rounded-[10px] ${
                              item?.userAvatar
                                ? 'object-cover'
                                : 'object-conatain p-[5px]'
                            }`}
                            src={item?.userAvatar || profileImage}
                          />
                        </div>
                        <p
                          title={item?.userName}
